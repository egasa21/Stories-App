...
private const val FILENAME_FORMAT = "dd-MMM-yyy"
...
fun uriToFile(selectedImg: Uri, context: Context) : File {
    ...
    val inputSystem = contentResolver.openInputStream(selectedImg) as InputStream
    val outputStream: OutputStream = FileOutputStream(myFile)
    val buf = ByteArray(1024)
    var len : Int

    while (inputSystem.read(buf).also { len = it } > 0) outputStream.write(buf, 0, len)
    outputStream.close()
    inputSystem.close()
